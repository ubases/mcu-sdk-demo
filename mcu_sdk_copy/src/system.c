...
static void UartSendProductInfo(void)
{
    ...
    sprintf((char *)buff, "{\"%s\":\"%s\",\"%s\":\"%s\",\"%s\":\"%s\"}", STR_PID, PRODUCT_ID, STR_VER, MCU_SOFTWARE_VER, STR_FLAG, PRODUCT_FLAG);
    InfoLog("buff = %s \r\n", buff);
 ...
 * @date    2022-06-22
 */
unsigned char UartCheckValue(unsigned char *dataBuff, unsigned char dataLen)
{
    unsigned char sum = 0;
    unsigned char n = 0;
    for(n = 0; n < dataLen; n++)
...
char ProcessReportDataInfo(unsigned char *FrameData, unsigned char data_len)
{
    ...
    for(count = 0; count < data_len; )
    {
        ...
        {
            ProcessDataCmd(cmd_id, &FrameData[count + 4]);
        }
        else
        {
